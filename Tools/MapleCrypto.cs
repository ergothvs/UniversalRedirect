...
namespace MapleLib.MapleCryptoLib
{
	...
	public class MapleCrypto
	{
        ...
        {
            //Diamondo25 left some dutch words here, changed them to english :p
            byte[] oldIV = new byte[4];
            Buffer.BlockCopy(mIV, 0, oldIV, 0, 4);
            for (int i = 0; i < pBuffer.Length; i++)
            {
                byte v7 = (byte)(pBuffer[i] ^ CryptoConstants.sShiftKey[mIV[0]]);
